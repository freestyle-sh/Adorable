...
describe("ProjectApplicationService", () => {
  ...
  it("creates GitHub-synced source repo, wrapper repo, VM, metadata, grants, and initial conversation", async () => {
    const { ProjectApplicationService } = await import(
      "@/lib/application/project-application-service"
    );

    const createGithubSyncedRepositoryCalls: Parameters<
      GitProvider["createGithubSyncedRepository"]
    >[0][] = [];
    const createRepositoryCalls: Parameters<
      GitProvider["createRepository"]
    >[0][] = [];
    ...
    const gitProvider = {
      async createRepository(input) {
        createRepositoryCalls.push(input);
        return { repoId: "wrapper-repo-id" };
      },
      async createGithubSyncedRepository(input) {
        createGithubSyncedRepositoryCalls.push(input);
        return { repoId: "github-source-repo-id" };
      },
      async grantWriteAccess() {
        throw new Error("grantWriteAccess should not be called");
      },
    ...
    const result = await service.createProject(
      {
        githubRepoName: "owner/source-project",
        requestedConversationTitle: "Initial conversation",
      },
      { access },
    );

    expect(createGithubSyncedRepositoryCalls).toEqual([
      {
        githubRepoName: "owner/source-project",
      },
    ]);
    expect(createRepositoryCalls).toEqual([
      {
        name: `${ADORABLE_WRAPPER_REPO_PREFIX}source-project`,
      },
    ]);
    expect(createRepositoryCalls[0]).not.toHaveProperty("import");
    expect(grantedGitRepoIds).toEqual([
      "github-source-repo-id",
      "wrapper-repo-id",
    ]);
    expect(createdForRepoIds).toEqual(["github-source-repo-id"]);
    expect(grantedVmIds).toEqual(["vm-id"]);
    expect(writeMetadataCalls).toEqual([
      {
        repoId: "wrapper-repo-id",
        metadata: {
          version: 2,
          sourceRepoId: "github-source-repo-id",
          vm: createdVm,
          conversations: [],
          deployments: [],
          productionDomain: null,
    ...
    expect(writeMetadataCalls[0]?.metadata).not.toHaveProperty("name");
    expect(createConversationCalls).toHaveLength(1);
    expect(createConversationCalls[0]).toMatchObject({
      repoId: "wrapper-repo-id",
      metadata: writeMetadataCalls[0]?.metadata,
